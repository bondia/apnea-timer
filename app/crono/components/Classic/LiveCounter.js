...
export default class LiveCounter extends React.PureComponent {
    ...
    render() {
        ...
        // current set styles
        let setStyle = baseStyles.headerText;
        setStyle = enums.SET_TYPE_HOLD === setType ? [setStyle, { color: COLOR_RED_NORMAL }] : setStyle;
        setStyle = enums.SET_TYPE_PREPARE === setType ? [setStyle, { color: COLOR_GREEN_NORMAL }] : setStyle;
            ...
            <View style={[this.props.style, baseStyles.wrapper]}>
                ...
                {current && mode !== enums.SET_MODE_FINISHED && (
                    <View style={baseStyles.headerBlock}>
                        ...
                        <TextComponent style={setStyle}>{secondsToTimeString(countdown)}</TextComponent>
                    </View>
                )}
                </View>
            </View>
