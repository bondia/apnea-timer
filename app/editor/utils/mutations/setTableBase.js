...
export default function setTableBase(state, value) {
    ...
    if (enums.TABLE_TYPE_CO2 === tableType) {
        state = state.updateIn(['sets'], s =>
            s.map(s => (s.get('type') == enums.SET_TYPE_HOLD ? s.set('duration', base) : s))
        );
    }
    // update sets for o2
    if (enums.TABLE_TYPE_O2 === tableType) {
        state = state.updateIn(['sets'], s =>
            s.map(s => (s.get('type') == enums.SET_TYPE_PREPARE ? s.set('duration', base) : s))
        );
    }
